...
include('class.uploader.php');
	...
	else
	{
		...
		$sitePath = "/home/lhuynh/lidar-web/www/Data/".$site;
		...
		$metadataPath = $sitePath."/".$type;
		if(!file_exists($metadataPath)){
			if (!mkdir($metadataPath, 0775, true)) {
				die('Failed to create folders...');
			}
		}
		...
		if($data['isComplete'])
		{
			...
			copy($metadataPath."/".$fileFullName, $metadataPath."/".$name."_metadata.xml");
			unlink($metadataPath."/".$fileFullName);
			
			echo json_encode($metadataPath."/".$name."_metadata.xml");
			
			//echo $metadataPath."/".$files['metas'][0]['name'];
			/*
