...
function GetPlatformList($project, $con)
{
    ...
    $sql = "Select flight.Platform as ID, platform.Name as Name from flight, platform " .
        "where flight.Platform = platform.ID and flight.Project = $project group by ID";
...
function GetSensorList($project, $platform, $con)
{
    ...
    $sql = "Select flight.Sensor as ID, sensor.Name as Name from flight, sensor " .
        "where flight.Sensor = sensor.ID and flight.Project = $project and flight.Platform = $platform group by ID";
...
{
    /*
    $sql = 	"Select replace(Date, '-', '/') as ID, replace(Date, '-', '/') as Name from flight ".
            "where Project = $project and Platform = $platform and Sensor = $sensor group by ID";
    */
    $sql = "Select replace(flight.Date, '-', '/') as ID, replace(flight.Date, '-', '/') as Name from flight, project " .
        "where flight.Project = $project and flight.Platform = $platform and flight.Sensor = $sensor group by ID";
...
function GetFlighList($project, $platform, $sensor, $date, $con)
{
    ...
    $sql = "select flight.*, project.Name as ProjectName,  platform.Name as PlatformName, sensor.Name as SensorName " .
        ...
        "and flight.Project = $project and flight.Platform = $platform and flight.Sensor = $sensor and flight.Date = '$date'";
