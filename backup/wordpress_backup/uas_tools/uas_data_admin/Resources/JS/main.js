...
function Add(type){
	if (CheckInput(type, null)){
		...
		//////
			...
			var type = $('#product_type_select').val();
			...
			if (data.length > 0)
			{
				...
				if (type == "flight"){
					...
					table +=			"<th>Crop</th>" +
						...
						"<th>Max Zoom</th>" +
						"<th>Default Zoom</th>" +
						"<th>Visualization Page</th>"; // add here link
				} else if (type == 'product'){
					table +=			"<th>TMS Path</th>";
				}
				...
				$.each(data,function(index,item)
				{
					...
					items+="<tr>" +
						"<td>" +
						"<input id='edit-" + type + "-" + item.ID +"' type='image' class='image-button edit-button' src='Resources/Images/edit.png' alt='Edit' onclick='Edit(" + item.ID +", \"" + type + "\"); return false;' title='Edit'>" +
						"<input id='confirmEdit-" + type + "-" + item.ID +"' type='image' class='image-button confirm-edit-button' src='Resources/Images/confirm.png' alt='Confirm' style='display:none' onclick='ConfirmEdit(" + item.ID +", \"" + type + "\"); return false;' title='Confirm'>" +
						"<input id='cancelEdit-" + type + "-" + item.ID +"' type='image' class='image-button cancel-edit-button' src='Resources/Images/cancel.png' alt='Cancel' style='display:none' onclick='CancelEdit(" + item.ID +", \"" + type + "\"); return false;' title='Cancel'>" +
			...
			if (data.length > 0)
			{
				...
				$.each(data,function(index,item)
				{
					if (type == "product"){
						...
						tmsButton = "<input id='tms-" + item.ID +"' type='image' class='image-button tms-button' " +
