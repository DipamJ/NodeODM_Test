...
function GetFinishedList(){
	$.ajax({
		...
		success: function(response) {
			...
			if (data.length > 0)
			{
				var table =	"<table id='product-table'>" +
					"<thead>" +
					"<tr>" +
					"<th>&nbsp;</th>" +
					"<th>&nbsp;</th>" +
				...
				$.each(data,function(index,item)
				{
					...
					items+= "<tr>" +
						...
						"<input id='download-" + item.ID +"' type='image' class='image-button download-button' src='Resources/Images/download.png' alt='Download' onclick='ViewDownload(\"" + item.DownloadPath + "\",\"" + item.UploadFolder + "\"); return false;' title='Download'>" +
						"</td>" +
						"<td>" +
