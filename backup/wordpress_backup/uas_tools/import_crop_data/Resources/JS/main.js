		...
		success: function(response) {
			if (response == "failed"){
				...
				if (data.length > 0)
				{
					...
					{
						items+= "<div id='field-" + FormatString(item) + "' onclick='SelectField(\"" + item + "\"); return false;' class='field-wrapper'>"  + item + "</div>";
					});
					items += "</div>"; 
...
function SelectField(field){
	...
	$("[id^=field-]").css("background-color","#ffffff");
	$("#field-"+ FormatString(field)).css("background-color","#e6ffe6");
