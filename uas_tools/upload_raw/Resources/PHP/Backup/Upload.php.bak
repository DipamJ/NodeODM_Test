...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
	...
	if (file_exists($chunk_file)) { //Chunk already exists, skip upload
		...
		if ($chunkNum == 1) {//first chunk, add a record to database
			...
			$sql = "INSERT INTO product_test (Flight, FileName, Type, MinZoom, MaxZoom, Zoom, EPSG, Size, ChunkCount, Status, Identifier, TempFolder) " .
				   "VALUES ($flightID, '$fileName', $typeID, $minZoom, $maxZoom, $zoom, $epsg, $size, $totalChunkNum, 'Uploading', '$identifier', '$temp_dir')";  
			_log($sql);
...
if (!empty($_FILES)) foreach ($_FILES as $file) {
    ...
    if(isset($_GET['resumableIdentifier']) && trim($_GET['resumableIdentifier'])!=''){
        //$temp_dir = '../Temp/'.$_GET['resumableIdentifier'];
		//$dest_dir = '../LAS/';
	    $temp_dir = SetTempFolderLocalPath().$_GET['resumableIdentifier'];
		...
		umask($old);
		$dest_dir = $path;
		$downloadPath = SetFolderHTMLPath().$projectName."/".$platformName."/".$sensorName."/".$date."/".$flightName."/".$typeName."/".$fileName;
	}
    ...
    if (!move_uploaded_file($file['tmp_name'], $dest_file)) {
        ...
        //createFileFromChunks($temp_dir, $dest_dir, $_GET['resumableFilename'],$_GET['resumableChunkSize'], $_GET['resumableTotalSize'],$_GET['resumableTotalChunks'], $_GET['resumableIdentifier']);
