...
$(document).ready(function(){
	...
	$(".normaldate").datepicker(); // Add select date funtionality to Date input box
...
function CreateResumableInstance(identifier){
	...
	//var minZoom = 0; //$("#min-zoom").val();
	//var maxZoom = 0; //$("#max-zoom").val();
	//var zoom    = 0; //$("#zoom").val();
	var epsg =  $("#epsg").val();
				...
				success: function(response) {
					...
					if (file == null || file["Status"] != "Finished"){
						...
						var rowStr = 	"<div id='resumable-" + count + "' style='height:30px; line-height: 30px'>" +
							"<div id='" + count + "-file-name' style='padding-top:5px; height: 30px ;width: 420px;float: left; border: 1px solid #CCCCCC;background:" + rowColor + ";'>" + fileName + "</div>" +
	...
	r.on('fileSuccess', function(file,message){
		...
		ConvertProduct(file.fileName, count, file.uniqueIdentifier, epsg);
		GetFinishedList();
		GetUnfinishedList();
	});
...
function ConvertProduct(fileName, index, identifier, epsg){
...
function GetFinishedList(){
	...
	// var projectName = "2016 Corpus Christi Cotton and Sorghum";
	// //var targetProjectStr = "Resources/PHP/GetFinishedList.php?project=" + projectName;
	//
	// $.ajax({
	// 	url: "/uas_tools/upload_geojson/V2/Resources/PHP/GetFinishedList.php",
	// 	type: "POST",
	// 	//dataType: 'text',
	// 	data: {
	// 		project: projectName
	...
	$.ajax({
		...
		success: function(response) {
			...
			if (data.length > 0)
			{
				...
				var table =	"<table id='product-table' style='width: 100% !important;'>" +
					"<thead>" +
					"<tr class='bg-dark text-white'>" +
					"<th style='border: none !important;'>&nbsp;</th>" +
					//"<th>&nbsp;</th>" +
					"<th style='border: none !important;'>File Name</th>" +
					"<th style='border: none !important;'>Type</th>" +
					...
					//"<th>Max Zoom</th>" +
					//"<th>Default Zoom</th>" +
					"<th style='border: none !important;'>EPSG</th>" +
					"</tr>" +
					"</thead>" +
				...
				$.each(data,function(index,item)
				{
					...
					items+= "<tr>" +
						"<td>" +
						// "<input id='tms-" + item.ID +"' type='image' class='image-button tms-button' src='Resources/Images/tms.png' alt='TMS' onclick='ViewTMS(\"" + item.TMSPath + "\"); return false;' title='TMS'>" +
						"<input style='padding: 7px !important; background: #5599ff; margin-right: 2px;' id='download-" + item.ID +"' type='image' class='' src='Resources/Images/download.png' alt='Download' onclick='ViewDownload(\"" + item.DownloadPath + "\",\"" + item.UploadFolder + "\"); return false;' title='Download'>" +
						// "</td>" +

						// Added
						"<input style='padding: 6px !important; background: #d9534f; margin-right: 2px;' id='delete-" + item.ID +"' type='image' class='image-button-delete-button' src='Resources/Images/delete.png' alt='Delete' onclick='Delete(" + item.ID + "); return false;' title='Delete'>" +
						"<input style='padding: 6px; background: #d9534f; display:none; margin-right: 2px;' id='confirmDelete-"  + item.ID +"' type='image' class='image-button-confirm-delete-button' src='Resources/Images/confirm.png' alt='Confirm' style='display:none' onclick='ConfirmDelete(" + item.ID + "); return false;' title='Confirm'>" +
						"<input style='padding: 6px; background: #ccc; display:none;' id='cancelDelete-"  + item.ID +"' type='image' class='image-button-cancel-delete-button' src='Resources/Images/cancel.png' alt='Cancel' style='display:none' onclick='CancelDelete(" + item.ID + "); return false;' title='Cancel'>" +
						"</td>" +
						// Added
						...
						// "</td>" +
						"<td style='overflow:hidden'>" +
						"<span id='" + item.ID +"-display-date'>" + item.Date + "</span>" +
						"</td>" +
...
//				var height = actualHeight < maxHeight ? actualHeight : maxHeight;
				$('#product-table').fxdHdrCol({
					...
					colModal: [
						...
						{ width: 120, align: 'center' }, // Download & Delete
						{ width: 200, align: 'center' }, // File Name
						{ width: 100, align: 'center' }, // Type
						{ width: 465,  align: 'center' }, // 300 Project
						// { width: 150,  align: 'center' }, // Platform
						// { width: 100,  align: 'center' }, // Sensor
						{ width: 100,  align: 'center' }, // Date
						//{ width: 100,  align: 'center' }, // Flight
						{ width: 75,  align: 'center' }, // EPSG
