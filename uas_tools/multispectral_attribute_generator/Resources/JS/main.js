                  ...
                  //console.log(flight_id);
                          ...
                          $.ajax({                                                                              //AJAX call to Orthomosiac php file that grabs orthomosiac files/info from database
                              ...
                              success: function (response) {                                                    //Grab response from AJAX call
                                  ...
                                  if (data.length > 0) {
                                    //console.log("not empty");                                                        //If respose is not empty
                                      $.each(data, function (index, item) {                                     //for each row/item in response. make value of item in dropdown menu equal to everything we need to pass to python file that generates attributes
                                          var name = item.FileName;
                                          name = name.split("_");
                                          var boundary_name = item.BoundaryFile;
                                          //console.log("ortho_total" + ortho_total);
                                          ...
                                          //console.log("boundary_name " + boundary_name);
                                          //console.log("selected_boundary " + selected_boundary);
...
function call_generateResults() {
  ...
  ortho_copy = [];
  chm_copy = [];
  ndvi = $('#ndvi_cb').is(":checked");
  ndre = $('#ndre_cb').is(":checked");
  msavi = $('#msavi_cb').is(":checked");
  // ch = $('#ch_cb').is(":checked");            //grab vars from fields
  // cv = $('#cv_cb').is(":checked");
  // exg = $('#exg_cb').is(":checked");          //grab vars from fields
  ...
  var ndvi_checked = $('#ndvi_cb').is(":checked");              //See if checkboxes have been checked
  var ndre_checked = $('#ndre_cb').is(":checked");
  var msavi_checked = $('#msavi_cb').is(":checked");
  //console.log("This is the Canopy Cover checkbox state: ");
  //console.log(ndvi_checked);
...
if (ndvi_checked == 'true'){
    attributes.push("ndvi_boundary");
  }
if (ndre_checked == 'true'){
    attributes.push("ndre_boundary");
  }
if (msavi_checked == 'true'){
    attributes.push("msavi_boundary");
  }
// if (exg_checked == 'true'){
...
$.ajax({                                          //AJAX call to generateResults.php
       ...
       data: {                                             // Data being sent to PHP file
         ...
         ndvi_ckd: ndvi_checked,
         ndre_ckd: ndre_checked,
         msavi_ckd: msavi_checked,
         // exg_ckd: exg_checked,
         // ch_ckd: ch_checked,
         // cv_ckd: cv_checked,
...
function generateZip(){
  ...
  var ndvi_checked = $('#ndvi_cb').is(":checked");            //check to see if checkboxes are checked
  //console.log(ndvi_checked);
...
// }
function ToggleRowData_caTable() {
    if (($("#ndvi_cb").is('checked')) && ($("#ndre_cb").is('checked')) && ($("#msavi_cb").is('checked'))) {
        $("#check-all-caTable").prop("checked", true);
    } else {
        $("#check-all-caTable").prop("checked", false);
    }
...
function ToggleAllRowData_caTable() {
    if($("#check-all-caTable").is(':checked')) {
        ...
        $("#ndvi_cb").prop("checked", true);
        $("#ndre_cb").prop("checked", true);
        $("#msavi_cb").prop("checked", true);
        // $("#exg_cb").prop("checked", true);
    }else {
        // $("#ch_cb").prop("checked", false);
        // $("#cv_cb").prop("checked", false);
        $("#ndvi_cb").prop("checked", false);
        $("#ndre_cb").prop("checked", false);
        $("#msavi_cb").prop("checked", false);
...
function Toggle_True_Checkboxes(){
  ...
  if(!$('#ndvi_cb').is(":checked")){
    $("#ndvi_cb").prop("checked", true);
  }

  if(!$('#ndre_cb').is(":checked")){
    $("#ndre_cb").prop("checked", true);
  }

  if(!$('#msavi_cb').is(":checked")){
    $("#msavi_cb").prop("checked", true);
  }
