...
def main():
    ...
    for x in selected_orthomosaic_list:
        ...
        if(sys.argv[4] == "true"):              # grab checkbox parameters and set them to boolean.
            csv_checked = True
        else:
            csv_checked = False
    ...
    #attributes array containing attribute boundary string extentions if selected
        ...
        if(ndvi_checked):
            attributes.append("ndvi_boundary")
        if(ndre_checked):
            attributes.append("ndre_boundary")
        if(msavi_checked):
            attributes.append("msavi_boundary")
        # if(exg_checked):
        ...
        if (ndvi_checked):
            dat_dirs.append("ndvi_rgb")
            att.append("ndvi_boundary")
        if (ndre_checked):
            dat_dirs.append("ndre_rgb")
            att.append("ndre_boundary")
        if (msavi_checked):
            dat_dirs.append("msavi_rgb")
            att.append("msavi_boundary")
        # if (exg_checked):
        #     dat_dirs.append("exg")
    ...
    #     attributes.append("cc_boundary")
    if (ndvi_checked):
        dat_array.append("ndvi_rgb")
        attributes.append("ndvi_boundary")
    if (ndre_checked):
        dat_array.append("ndre_rgb")
        attributes.append("ndre_boundary")
    if (msavi_checked):
        dat_array.append("msavi_rgb")
        attributes.append("msavi_boundary")
    # if (exg_checked):
    ...
    ndviZip = results_dir + "/" + "ndviZip"
    ndreZip = results_dir + "/" + "ndreZip"
    msaviZip = results_dir + "/" + "msaviZip"
    ...
    for i in attributes:
        ...
        if(os.path.exists(attribute_path)):
            ...
            if(i == "ndvi_boundary"):
                shutil.make_archive(ndviZip, 'zip', attribute_path)
            if(i == "ndre_boundary"):
                shutil.make_archive(ndreZip, 'zip', attribute_path)
            if(i == "msavi_boundary"):
                shutil.make_archive(msaviZip, 'zip', attribute_path)
            # if(i == "exg_boundary"):
