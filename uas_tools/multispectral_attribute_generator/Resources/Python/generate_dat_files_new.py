...
from gen_ndvi_boundary import get_ndvi
from gen_ndre_boundary import get_ndre
from gen_msavi_boundary import get_msavi
...
def main():
    ...
    for x in selected_orthomosaic_list:
        ...
        selected_boundary = sys.argv[3].split("::")
        ...
        if(sys.argv[4] == "true"):                  # bools for checkboxes that are passed in as string so we need to set checkboxes to reset them to boolean vars.
            ndvi_checked = True
        else:
            ndvi_checked = False
        ...
        if(sys.argv[6] == "true"):                  # bools for checkboxes that are passed in as string so we need to set checkboxes to reset them to boolean vars.
            msavi_checked = True
        else:
            msavi_checked = False
    ...
    # Call main functions to set global variables, generate attributes and remove files that are no longer needed.
        ...
        generate_attributes(object_handle, selected_orthomosaic_FileName_noExt, selected_orthomosaic_EPSG, selected_boundary_FileName_noExt, ndvi_checked, ndre_checked, msavi_checked)
    ...
    if (len(selected_orthomosaic_list) > 1):
        ...
        merge_shp(selected_boundary_FilePath, selected_orthomosaic_EPSG, ndvi_checked, ndre_checked, msavi_checked)
...
def select_out_folder(object_handle, path_to_results_folder):
    ...
    ndvi_out_dir = os.path.join(out_dir, "ndvi")
    ndre_out_dir = os.path.join(out_dir, "ndre")
    msavi_out_dir = os.path.join(out_dir, "msavi")
    #print(ndvi_out_dir)
    ...
    results_folders = [
                       #cc_out_dir,                  # put paths of directories into an array
                       ndvi_out_dir,
                       ndre_out_dir,
                       msavi_out_dir
                       # grvi_out_dir,
...
def generate_vis(object_handle, selected_orthomosaic_FileName_noExt):
    ...
    for f in files:
        ...
        # Read bands
        # blue = in_img.img[0,:,:].astype(np.uint8)
        # green = in_img.img[1,:,:].astype(np.uint8)
        # panchro = in_img.img[2,:,:].astype(np.uint8)
        # red = in_img.img[3,:,:].astype(np.uint8)
        # nir = in_img.img[5,:,:].astype(np.uint8)
        # alpha = in_img.img[6,:,:].astype(np.uint8)
        blue = in_img.img[0,:,:].astype(np.float32)
        green = in_img.img[1,:,:].astype(np.float32)
        panchro = in_img.img[2,:,:].astype(np.float32)
        red = in_img.img[3,:,:].astype(np.float32)
        rededge = in_img.img[4,:,:].astype(np.float32)
        nir = in_img.img[5,:,:].astype(np.float32)
        alpha = in_img.img[6,:,:].astype(np.float32)
        ...
        msavi = 0.5*(2*nir+1-((2*nir+1)**2-8*(nir-red))**(1.0/2))
        ...
        object_handle.write("Saving NDVI... " + " \n")
        out_fn_ndvi = os.path.join(out_dir, 'ndvi', selected_orthomosaic_FileName_noExt + '_ndvi.dat')
        driver_ndvi = gdal.GetDriverByName("ENVI")
        outds_ndvi = driver_ndvi.Create(out_fn_ndvi, in_img.ds.RasterXSize, in_img.ds.RasterYSize, 2, gdal.GDT_Float32)
        outds_ndvi.SetGeoTransform(in_img.ds.GetGeoTransform())
        outds_ndvi.SetProjection(in_img.ds.GetProjection())
        outds_ndvi.GetRasterBand(1).WriteArray(ndvi)
        outds_ndvi.GetRasterBand(2).WriteArray(alpha)

        object_handle.write("Saving NDRE... " + " \n")
        out_fn_ndre = os.path.join(out_dir, 'ndre', selected_orthomosaic_FileName_noExt + '_ndre.dat')
        driver_ndre = gdal.GetDriverByName("ENVI")
        outds_ndre = driver_ndre.Create(out_fn_ndre, in_img.ds.RasterXSize, in_img.ds.RasterYSize, 2, gdal.GDT_Float32)
        outds_ndre.SetGeoTransform(in_img.ds.GetGeoTransform())
        outds_ndre.SetProjection(in_img.ds.GetProjection())
        outds_ndre.GetRasterBand(1).WriteArray(ndre)
        outds_ndre.GetRasterBand(2).WriteArray(alpha)

        object_handle.write("Saving MSAVI... " + " \n")
        out_fn_msavi = os.path.join(out_dir, 'msavi', selected_orthomosaic_FileName_noExt + '_msavi.dat')
        driver_msavi = gdal.GetDriverByName("ENVI")
        outds_msavi = driver_msavi.Create(out_fn_msavi, in_img.ds.RasterXSize, in_img.ds.RasterYSize, 2, gdal.GDT_Float32)
        outds_msavi.SetGeoTransform(in_img.ds.GetGeoTransform())
        outds_msavi.SetProjection(in_img.ds.GetProjection())
        outds_msavi.GetRasterBand(1).WriteArray(msavi)
        outds_msavi.GetRasterBand(2).WriteArray(alpha)
...
def generate_attributes(object_handle, selected_orthomosaic_FileName_noExt, selected_orthomosaic_EPSG, selected_boundary_FileName_noExt, ndvi_checked, ndre_checked, msavi_checked):
    ...
    if(ndvi_checked):
        ndvi_shp_path = os.path.join(out_dir, 'ndvi_boundary', ('ndvi_boundary_' + selected_orthomosaic_FileName_noExt + ".shp"))
        if(os.path.exists(ndvi_shp_path)):
            print("The NDVI shape file already exists! Skipping NDVI attribute generation!")
            pass
        else:
            get_ndvi(selected_orthomosaic_EPSG, shp_file, out_dir, selected_orthomosaic_FileName_noExt, object_handle)                        # call to python function to calculate Excess Greeness (ndvi)

            ndvi_geojson_path = os.path.join(out_dir, 'ndvi_boundary', ('ndvi_boundary_' + selected_orthomosaic_FileName_noExt + '.geojson'))
            command = f"ogr2ogr -f geojson {ndvi_geojson_path} {ndvi_shp_path}"                           #command to convert shape file to geojson file
            if(os.system(command) == 0):                                                                #if the command was success, print success message. else, print error message
                print("The shape file was successfully converted to a geojson file!")

                ndvi_geojson_file = geopandas.read_file(ndvi_geojson_path)                                                                              # read ndvi geojson file to variable
                ndvi_geojson_file_ESPG = ndvi_geojson_file.crs                                                                                 # check the CRS of the geojson file.

                print(f"ndvi_geojson_file_ESPG: {ndvi_geojson_file_ESPG}")

                if (ndvi_geojson_file_ESPG != 4326):                             #If GeoJSON file EPSG not equal to 4326, convert the file.
                    ndvi_geojson_converted_path = os.path.join(out_dir, 'ndvi_boundary', ('ndvi_boundary_' + selected_boundary_FileName_noExt + "_converted"+ '.geojson'))     # path for converted ndvi geoJSON file
                    command = f"ogr2ogr -s_srs {ndvi_geojson_file_ESPG} -t_srs epsg:4326 -f GEOJSON {ndvi_geojson_converted_path} {ndvi_geojson_path}"
                    if(os.system(command) == 0):                                                            #if the command was success, print success message. else, print error message
                        print("The GeoJSON file was successfully converted to the EPSG value 3426!")
                        os.remove(ndvi_geojson_path)
                        shutil.move(ndvi_geojson_converted_path, ndvi_geojson_path)
                    else:
                        print("ERROR: There was an error trying to convert the NDVI GeoJSON file to the correct EPSG!")
                else:
                    print("The GeoJSON file already has the correct EPSG value (4326).")
            else:
                print("ERROR: there was an error trying to create the GeoJSON file!")

            ndvi_csv_path = os.path.join(out_dir, 'ndvi_boundary', ('ndvi_boundary_' + selected_orthomosaic_FileName_noExt + '.csv'))
            command = f"ogr2ogr -f CSV {ndvi_csv_path} {ndvi_shp_path}"                                                                    # command to convert shape file to CSV file format.
            if(os.system(command) == 0):                                                                                                 # if the command was success, print success message. else, print error message
                print("The file was successfully converted to a CSV file!!!!!!!!")
            else:
                print("ERROR: There was an error trying to convert the shape file to the CSV file format.")

            ndvi_xlsx_path = os.path.join(out_dir, 'ndvi_boundary', ('ndvi_boundary_' + selected_orthomosaic_FileName_noExt + '.xlsx'))
            command = f"ogr2ogr -f xlsx {ndvi_xlsx_path} {ndvi_shp_path}"                                                                          # command to convert shape file to xlsx file format.
            if(os.system(command) == 0):                                                                                                    # if the command was success, print success message. else, print error message
                print("The file was successfully converted to a xlsx file!!!!!!!!")
            else:
                print("ERROR: There was an error trying to convert the shape file to the xlsx file format.")

        object_handle.write("Done with NDVI attributes " + " \n")


    if(ndre_checked):
        ndre_shp_path = os.path.join(out_dir, 'ndre_boundary', ('ndre_boundary_' + selected_orthomosaic_FileName_noExt + ".shp"))
        if(os.path.exists(ndre_shp_path)):
            print("The NDRE shape file already exists! Skipping NDRE attribute generation!")
            pass
        else:
            get_ndre(selected_orthomosaic_EPSG, shp_file, out_dir, selected_orthomosaic_FileName_noExt, object_handle)                        # call to python function to calculate Excess Greeness (ndre)

            ndre_geojson_path = os.path.join(out_dir, 'ndre_boundary', ('ndre_boundary_' + selected_orthomosaic_FileName_noExt + '.geojson'))
            command = f"ogr2ogr -f geojson {ndre_geojson_path} {ndre_shp_path}"                           #command to convert shape file to geojson file
            if(os.system(command) == 0):                                                                #if the command was success, print success message. else, print error message
                print("The shape file was successfully converted to a geojson file!")

                ndre_geojson_file = geopandas.read_file(ndre_geojson_path)                                                                              # read ndre geojson file to variable
                ndre_geojson_file_ESPG = ndre_geojson_file.crs                                                                                 # check the CRS of the geojson file.

                print(f"ndre_geojson_file_ESPG: {ndre_geojson_file_ESPG}")

                if (ndre_geojson_file_ESPG != 4326):                             #If GeoJSON file EPSG not equal to 4326, convert the file.
                    ndre_geojson_converted_path = os.path.join(out_dir, 'ndre_boundary', ('ndre_boundary_' + selected_boundary_FileName_noExt + "_converted"+ '.geojson'))     # path for converted ndre geoJSON file
                    command = f"ogr2ogr -s_srs {ndre_geojson_file_ESPG} -t_srs epsg:4326 -f GEOJSON {ndre_geojson_converted_path} {ndre_geojson_path}"
                    if(os.system(command) == 0):                                                            #if the command was success, print success message. else, print error message
                        print("The GeoJSON file was successfully converted to the EPSG value 3426!")
                        os.remove(ndre_geojson_path)
                        shutil.move(ndre_geojson_converted_path, ndre_geojson_path)
                    else:
                        print("ERROR: There was an error trying to convert the NDRE GeoJSON file to the correct EPSG!")
                else:
                    print("The GeoJSON file already has the correct EPSG value (4326).")
            else:
                print("ERROR: there was an error trying to create the GeoJSON file!")

            ndre_csv_path = os.path.join(out_dir, 'ndre_boundary', ('ndre_boundary_' + selected_orthomosaic_FileName_noExt + '.csv'))
            command = f"ogr2ogr -f CSV {ndre_csv_path} {ndre_shp_path}"                                                                    # command to convert shape file to CSV file format.
            if(os.system(command) == 0):                                                                                                 # if the command was success, print success message. else, print error message
                print("The file was successfully converted to a CSV file!!!!!!!!")
            else:
                print("ERROR: There was an error trying to convert the shape file to the CSV file format.")

            ndre_xlsx_path = os.path.join(out_dir, 'ndre_boundary', ('ndre_boundary_' + selected_orthomosaic_FileName_noExt + '.xlsx'))
            command = f"ogr2ogr -f xlsx {ndre_xlsx_path} {ndre_shp_path}"                                                                          # command to convert shape file to xlsx file format.
            if(os.system(command) == 0):                                                                                                    # if the command was success, print success message. else, print error message
                print("The file was successfully converted to a xlsx file!!!!!!!!")
            else:
                print("ERROR: There was an error trying to convert the shape file to the xlsx file format.")

        object_handle.write("Done with NDRE attributes " + " \n")


    if(msavi_checked):
        msavi_shp_path = os.path.join(out_dir, 'msavi_boundary', ('msavi_boundary_' + selected_orthomosaic_FileName_noExt + ".shp"))
        if(os.path.exists(msavi_shp_path)):
            print("The MSAVI shape file already exists! Skipping MSAVI attribute generation!")
            pass
        else:
            get_msavi(selected_orthomosaic_EPSG, shp_file, out_dir, selected_orthomosaic_FileName_noExt, object_handle)                        # call to python function to calculate Excess Greeness (msavi)

            msavi_geojson_path = os.path.join(out_dir, 'msavi_boundary', ('msavi_boundary_' + selected_orthomosaic_FileName_noExt + '.geojson'))
            command = f"ogr2ogr -f geojson {msavi_geojson_path} {msavi_shp_path}"                           #command to convert shape file to geojson file
            if(os.system(command) == 0):                                                                #if the command was success, print success message. else, print error message
                print("The shape file was successfully converted to a geojson file!")

                msavi_geojson_file = geopandas.read_file(msavi_geojson_path)                                                                              # read msavi geojson file to variable
                msavi_geojson_file_ESPG = msavi_geojson_file.crs                                                                                 # check the CRS of the geojson file.

                print(f"msavi_geojson_file_ESPG: {msavi_geojson_file_ESPG}")

                if (msavi_geojson_file_ESPG != 4326):                             #If GeoJSON file EPSG not equal to 4326, convert the file.
                    msavi_geojson_converted_path = os.path.join(out_dir, 'msavi_boundary', ('msavi_boundary_' + selected_boundary_FileName_noExt + "_converted"+ '.geojson'))     # path for converted msavi geoJSON file
                    command = f"ogr2ogr -s_srs {msavi_geojson_file_ESPG} -t_srs epsg:4326 -f GEOJSON {msavi_geojson_converted_path} {msavi_geojson_path}"
                    if(os.system(command) == 0):                                                            #if the command was success, print success message. else, print error message
                        print("The GeoJSON file was successfully converted to the EPSG value 3426!")
                        os.remove(msavi_geojson_path)
                        shutil.move(msavi_geojson_converted_path, msavi_geojson_path)
                    else:
                        print("ERROR: There was an error trying to convert the MSAVI GeoJSON file to the correct EPSG!")
                else:
                    print("The GeoJSON file already has the correct EPSG value (4326).")
            else:
                print("ERROR: there was an error trying to create the GeoJSON file!")

            msavi_csv_path = os.path.join(out_dir, 'msavi_boundary', ('msavi_boundary_' + selected_orthomosaic_FileName_noExt + '.csv'))
            command = f"ogr2ogr -f CSV {msavi_csv_path} {msavi_shp_path}"                                                                    # command to convert shape file to CSV file format.
            if(os.system(command) == 0):                                                                                                 # if the command was success, print success message. else, print error message
                print("The file was successfully converted to a CSV file!!!!!!!!")
            else:
                print("ERROR: There was an error trying to convert the shape file to the CSV file format.")

            msavi_xlsx_path = os.path.join(out_dir, 'msavi_boundary', ('msavi_boundary_' + selected_orthomosaic_FileName_noExt + '.xlsx'))
            command = f"ogr2ogr -f xlsx {msavi_xlsx_path} {msavi_shp_path}"                                                                          # command to convert shape file to xlsx file format.
            if(os.system(command) == 0):                                                                                                    # if the command was success, print success message. else, print error message
                print("The file was successfully converted to a xlsx file!!!!!!!!")
            else:
                print("ERROR: There was an error trying to convert the shape file to the xlsx file format.")

        object_handle.write("Done with MSAVI attributes " + " \n")


# def merge_shp(selected_boundary_FilePath, selected_orthomosaic_EPSG, cc_checked, exg_checked, ch_checked, cv_checked):
def merge_shp(selected_boundary_FilePath, selected_orthomosaic_EPSG, ndvi_checked, ndre_checked, msavi_checked):
    ...
    #     dat_dirs.append("cv")
    if (ndvi_checked):
        dat_dirs.append("ndvi")
    if (ndre_checked):
        dat_dirs.append("ndre")
    if (msavi_checked):
        dat_dirs.append("msavi")
    ...
    for d in dat_dirs:
        ...
        for o in ortho_name_array:            # Iterate through the length of string extensions for the directories
            ...
            if (count == 0 or chm_counter == 1):                                     # create temp folders for merged SHP results
                if os.path.exists(temp_dat_folder):
                    ...
                    os.makedirs(temp_dat_folder)
            if (d == 'cc_rgb' or d == "exg" or d == "ndvi" or d == "ndre" or d == "msavi"):                                     # Move dat files for CC/EXG results
        ...
        print("B")
        if (d == 'cc_rgb' or d == 'exg' or d == 'ndvi' or d == 'ndre' or d == 'msavi'):
